...
/*   Updated: 2020/06/14 23:43:20 by wbertoni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_error ft_get_ceiling_floor_color(t_file *file, char **info)
{
	char **rgb;
	int size;
...
static t_error ft_get_texture_path(t_vars *vars, t_file *file, char **info)
{
	int size;
...
static t_error ft_get_resolution(t_file *file, char **info)
{
	int size;
...
t_error ft_get_info_value(t_vars *vars, t_file *file, char **info)
{
		...
		{
			if ((e = ft_get_texture_path(vars, file, info)) != noerror)
				return (e);
		}
