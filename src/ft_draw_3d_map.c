...
/*   Updated: 2020/12/28 18:45:18 by wbertoni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_draw_3d_map(t_game *game, t_img *img)
{
	...
	while (i < (game->cubfile->width / WALL_STRIP_WIDTH))
	{
		...
		wall_height = (g_tile_size / perpDistance) * player_distance;
		...
		> game->cubfile->height) ? game->cubfile->height : game->rays[i]->wall_bottom;
