...
/*   Updated: 2020/06/15 00:32:05 by wbertoni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

static t_error ft_check_cubfile(t_file *file)
{
...
static t_error ft_get_player_position(t_file *file)
{
	int i;
	int j;
...
t_error ft_get_cubfile_value(t_vars *vars, t_file *file, char *line)
{
	...
	{
		info = ft_split(line, ' ');
		if ((e = ft_get_info_value(vars, file, info)) != noerror)
			return (e);
	}
	return (noerror);
}
